 ...
 * 融资中的项目
 */
public class WaitingProjectFragment extends BaseFragment implements SwipyRefreshLayout.OnRefreshListener {
...
//            ACache aCache = ACache.get(getActivity());
            if (!NetWorkUtils.getNetWorkType(getActivity()).equals(NetWorkUtils.NETWORK_TYPE_DISCONNECT)) {
                //����
                try {
                    body = OkHttpUtils.doGet(Constant.BASE_URL + Constant.PHONE + Constant.PROJECT_TITLE + "2/" + pageNo + "/", getActivity());
                    Log.e(TAG, body);
                    aCache.put("WaitingProject" + pageNo, body, 1 * ACache.TIME_DAY);
                } catch (Exception e) {
                    e.printStackTrace();
                ...
                body = aCache.getAsString("WaitingProject" + pageNo);
                if (body == null) {
                    return null;
                }
            }
        ...
        protected void onPostExecute(FinishFinacingBean aVoid) {
            ...
            if (aVoid != null) {
                ...
                if (aVoid.getCode() == 2 && finish_list.size() == 0) {
                    blankPage();
                    return;
                }
